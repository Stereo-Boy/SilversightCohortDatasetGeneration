...
% if the flag PR_ECRF_DATA is set to false, then reprocess the processed PR file just to convert scores from log to percentage 
    ...
    if PR_ECRF_DATA==1  || PR_ECRF_DATA==3
        ...
        if pr_file_found(1)
            ...
            RAW_PR_FILE_PATH = fullfile(ECRF_DATA_DIR,FILE_NAME);
            ...
            writetable(table_to_export,fullfile(PROCESSED_DATA_DIR, 'PR_Raw_Processed.xlsx'))
        end
    end
    if  PR_ECRF_DATA==2 || PR_ECRF_DATA==3
        ...
        if pr_file_found(1)
            ...
            FILE_NAME = FILE_NAME(1).name; % take only one PR data file in case there multiple files
            PROCESSED_PR_FILE_PATH = fullfile(LOCAL_DATA_DIR, FILE_NAME);
            ...
            writetable(table_to_export,fullfile(PROCESSED_DATA_DIR, 'PR_Local_Processed.xlsx'))
        end
    end
