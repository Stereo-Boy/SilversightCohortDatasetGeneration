...
disp('  Begin data processing for OK_FIX ');
    ...
    for k = 1:length(file)
        FileData = load(fullfile(file(k).folder,file(k).name)); %load file
    ...
    for k = 1:length(file)
        FileData = load(fullfile(file(k).folder,file(k).name));
        filenames = file(k).name;
    ...
    for s = 1:size(T,2)
        ...
        writetable(T{s}, fullfile(PROCESSED_DATA_DIR, 'OK_FIX.xls'),'FileType','spreadsheet','Sheet',s);
    end
