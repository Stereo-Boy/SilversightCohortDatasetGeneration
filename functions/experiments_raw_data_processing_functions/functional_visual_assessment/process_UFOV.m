...
% 2- Ching Shuang table if the flag  UFOV_ECRF_DATA is set to false 
    ...
    if UFOV_ECRF_DATA == 1 || UFOV_ECRF_DATA== 3
        ...
        if ufov_file_found(1)
            ufov_TABLE = readtable([FILE_NAME(1).folder '\' FILE_NAME(1).name]);
            ...
            writetable(ufov_TABLE, [PROCESSED_DATA_DIR 'UFOV_ecrf_processed' '.xls'],'FileType','spreadsheet');
    ...
    if  UFOV_ECRF_DATA == 2 || UFOV_ECRF_DATA== 3 % using Shing Shuang UFOV table
        ...
        writetable(new_UFOV_TABLE, [PROCESSED_DATA_DIR 'UFOV_local_reprocessed'  '.xls'],'FileType','spreadsheet');          
    end
