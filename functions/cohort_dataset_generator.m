...
% adding paths where the cohort generation functions are 
addpath('cohort_dataset_generation_functions/functional_visual_assessment');
addpath('cohort_dataset_generation_functions/functional_hearing_assessment');
addpath('cohort_dataset_generation_functions/neuropsychological_examination');
addpath('cohort_dataset_generation_functions');
addpath('helpers');
...
if EXECUTE_ETDRS 
    [etdrs_4m_TABLE] = getETDRSTable(); 
end % j.1.ETDRS
if EXECUTE_DM dm_TABLE = getDMTable(); end % j.2.DM

%%% 2-Functional hearing assessment
if EXECUTE_AUDIOGGRAM audio_TABLE=getAudioTable(); end
%%% 3-Neuropsychological examination
if EXECUTE_COG_T cog_T_TABLE=getCOGTTable(); end
if EXECUTE_VTS wts_TABLE = getWTSTable(); end % age better be moved to different approach 
...
% audio assessment
if EXECUTE_AUDIOGGRAM cohort_DATASET_TABLE = outerjoin(cohort_DATASET_TABLE,audio_TABLE,'Keys','Identifiant','MergeKeys',true); end
...
if EXECUTE_VTS cohort_DATASET_TABLE = outerjoin(cohort_DATASET_TABLE,wts_TABLE,'Keys','Identifiant','MergeKeys',true); end   
...
writetable(cohort_DATASET_TABLE,[PROCESSED_DATA_DIR 'CohortDataSet_' strrep(datestr(datetime('today'),'dd-mm-yyyy'), '-','')], 'FileType','spreadsheet');
