...
try
    ...
    if PR_ECRF_DATA==2 || PR_ECRF_DATA==3 %local data
        FILE_NAME = list_files(LOCAL_DATA_DIR,'*BAT*.*',1); % look for files that have BAT in thier names
        if numel(FILE_NAME)>1
           disp('warning: more than one BAT file in local folder: please check. In the meantime, we take the first one only')
           FILE_NAME=FILE_NAME(1);
        end
        if check_file(FILE_NAME{1},0)
            warning off
            bat_TABLE = readtable(FILE_NAME{1},'Range', 'A:N');
            warning on
...
%                end 
%             end
            bat_TABLE.DateRealisation=datetime(bat_TABLE.DateRealisation,'InputFormat','dd-MMM-YYYY');
            bat_TABLE.DateRealisation=cellstr(datestr(bat_TABLE.DateRealisation,'dd/mm/yyyy'));
            ...
            disp(['File does not exist: we skip ',FILE_NAME{1}])
        end
    end
