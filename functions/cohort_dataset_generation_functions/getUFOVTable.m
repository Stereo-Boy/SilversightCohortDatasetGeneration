...
%GETUVOFTABLE
    ...
    if UFOV_ECRF_DATA == 1 || UFOV_ECRF_DATA == 3  
        ...
        if ufov_file_found(1)
            ufov_TABLE = readtable(fullfile(FILE_NAME(1).folder,FILE_NAME(1).name)); % no need to remove duplicities since it was handled in the generation
    ...
    if UFOV_ECRF_DATA == 3
        ...
        writetable(ufov_TABLE, fullfile(PROCESSED_DATA_DIR,'UFOV_merged.xls'),'FileType','spreadsheet');      
    end
