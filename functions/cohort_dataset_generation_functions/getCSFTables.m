...
%GETCSFTABLES Import CSF BINO and MONO Tables
    ...
    csf_bino_TABLE = readtable(fullfile(FILE_NAME(1).folder,FILE_NAME(1).name), 'ReadVariableNames', false); % ReadVariableNames is false cause tables files do not have headers
    ...
    csf_mono_TABLE = readtable(fullfile(FILE_NAME(1).folder,FILE_NAME(1).name), 'ReadVariableNames', false); % ReadVariableNames is false cause tables files do not have headers
    csf_mono_TABLE=removeDuplicates(csf_mono_TABLE);
